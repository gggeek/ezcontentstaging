    ...
    public function createResult()
    {
        ...
        if( ( $res = $cache->restore( $controllerCacheId ) ) === false )
        {
            try
            {
                ...
                // gg: fix for issue #018940 - note that we might, when in debug mode, recover the text output so far and use it for troubleshooting purposes
                $errrorReporting = error_reporting();
                error_reporting( 0 );
                ob_start();
                $res = parent::createResult();
                ob_end_clean();
                error_reporting( $errrorReporting );
